...
The full categorized tool surface, current count, and per-tool signatures are canonical in [`ai_docs/domains/python-refactor/reference.md`](../domains/python-refactor/reference.md) — do not duplicate the list or the count here; it drifts every time a tool is added or removed. See also [`ai_docs/domains/python-refactor/mcp-checklist.md`](../domains/python-refactor/mcp-checklist.md) for a Goal/Validation/Chaining prompt example per tool.
