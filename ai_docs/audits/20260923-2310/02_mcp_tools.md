...
| mcp-surface-unknown-args-ignored | low | 98/101 tools with params accept and silently drop unknown keys (no `additionalProperties:false`); a typo in an optional param (`include_contxt`, `aply`) silently falls back to the default. | `call_hierarchy{…, __unknown_extra__:1}` → ok | SDK-generated arg models (all tool schemas) |
