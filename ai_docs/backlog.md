...
| id | pri | deps | do |
|----|-----|------|-----|
| pyright-version-mismatch | Medium | none | `pyrightconfig.json` pins `pythonVersion: "3.13"` while `pyproject.toml` has `requires-python >= 3.14`, ruff `target-version = "py314"`, and mypy `python_version = "3.14"`. Pyright type-checks against a stale grammar, masking 3.14-only syntax and stdlib drift. Bump pyright pin to `"3.14"` and add a regression check (script or CI grep) that all four version pins move together. Anchors: `pyrightconfig.json`, `pyproject.toml`. Evidence: work-search audit (2026-05-27 discovery-sweep). |
