# Human Documentation

Entry point for human-readable documentation for the Python Refactor MCP server.

| Doc | Purpose |
|-----|---------|
| [setup.md](setup.md) | Installation, configuration, validation commands, MCP client setup, executable build, troubleshooting |
| [usage.md](usage.md) | Running the server, usage examples, architecture overview |
| [tool-reference.md](tool-reference.md) | Full 106-tool reference table by category |
| [PRIVACY.md](../PRIVACY.md) | Privacy policy |

Agent-facing documentation lives in [ai_docs/](../ai_docs/README.md).
