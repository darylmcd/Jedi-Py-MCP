...
- **Maintenance:** Refreshed the authoritative lock to mypy 2.3.1, LibCST 1.9.0, Pydantic 2.13.5, Pyright 1.1.411, PyInstaller 6.22.2, pytest 9.1.1, and Ruff 0.16.5. Hosted third-party actions are pinned to immutable commit SHAs, and the repository requires uv 0.11.1 so dependency resolution is reproducible.
...
- **Maintenance:** Migrated the mypy dev dependency to 2.x (`mypy>=2.0`) and removed the `[[tool.mypy.overrides]]` block that blanket-waived `type-arg`/`unused-ignore` for `server.py` + `tool_registry.py`. MCP SDK 2's `Context` defaults its remaining generic parameters, so handlers now use the framework's bare injected type directly with no runtime/type-checking alias split. `mypy --strict` passes 0-error under 2.x; future `type-arg` errors in those two modules now surface instead of being silently waived. Closes `mypy-2x-migration`.
