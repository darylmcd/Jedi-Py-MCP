...
- **Fixed (packaging/CI):** Source installs now constrain the MCP SDK to the supported 1.x API (`mcp>=1.27,<2`), and hosted CI syncs the committed `uv.lock` instead of resolving open-ended dependencies with pip. MCP 2.1.1 had removed the 1.x `FastMCP`/`Context` export shape and changed tool annotations, preventing clean installs from passing type checks or starting reliably.
